...
private:
    ...
    uint16_t _port;
    int _timeout;                                       // 这是非活跃连接的统计时间 --- 多长时间五通信就是非活跃连接
    ...
    using MessageCallback = std::function<void(const PtrConnection &, Buffer *)>;
    using ClosedCallback = std::function<void(const PtrConnection &)>;
    using AnyEventdCallback = std::function<void(const PtrConnection &)>;
    using Functor = std::function<void()>;
    ConnectedCallback _connected_callback;
    MessageCallback _message_callback;
    ClosedCallback _closed_callback;
    AnyEventdCallback _event_callback;
...
public:
    ...
    void SetMessageCallback(const MessageCallback &cb) { _message_callback = cb; }
    void SetClosedCallback(const ClosedCallback &cb) { _closed_callback = cb; }
    void SetAnyEventdCallback(const AnyEventdCallback &cb) { _event_callback = cb; }
...
/*定时器中有一个_timers成员，定时器信息的操作有可能在多线程中进行，因此需要考虑线程安全问题*/
